...
def get_train_mse(R, user_vecs, movie_vecs):
    ...
    predict_R = user_vecs @ movie_vecs.T
    R[indices] = predict_R[indices]
    mse_loss = np.sum((R-predict_R)**2)
    return mse_loss
...
def update_user_vecs(user_vecs, movie_vecs, R, user_rated_idxs):
    ...
    for i in range(user_vecs.shape[0]):
        ...
        mat_sum = np.zeros((movie_vecs.shape[1], movie_vecs.shape[1]))
        vec_sum = np.zeros(movie_vecs.shape[1])
        for j in idxs:
            mat_sum += np.outer(movie_vecs[j], movie_vecs[j])
            vec_sum += R[i,j] * movie_vecs[j]
        # outer_product = np.multiply.outer(movie_vecs[idxs], movie_vecs[idxs]).reshape(-1, movie_vecs.shape[1], movie_vecs.shape[1])
        # user_vecs[i] = np.linalg.inv(np.sum(outer_product, axis=0)+ 1) @ (movie_vecs[idxs].T @ R[i, idxs])
        user_vecs[i] = np.linalg.inv(mat_sum + 1) @ vec_sum
    return user_vecs
...
def update_movie_vecs(user_vecs, movie_vecs, R, movie_rated_idxs):
    ...
    for j in range(movie_vecs.shape[0]):
        ...
        mat_sum = np.zeros((user_vecs.shape[1], user_vecs.shape[1]))
        vec_sum = np.zeros(user_vecs.shape[1])
        for i in idxs:
            mat_sum += np.outer(user_vecs[i], user_vecs[i])
            vec_sum += R[i,j] * user_vecs[i]
        # outer_product = np.outer(user_vecs[idxs], user_vecs[idxs]).reshape(-1, user_vecs.shape[1], user_vecs.shape[1])
        # movie_vecs[j] = np.linalg.inv(np.sum(outer_product, axis=0) + 1) @ (R[idxs, j] @ user_vecs[idxs])
        movie_vecs[j] = np.linalg.inv(mat_sum + 1) @ vec_sum
